...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _handleDeepLink() async {
    ...
    // Extract latitude and longitude from the deep link if available
...
print(latString);
  ...
  void _onMapCreated(MaplibreMapController controller) {
    ...
    Fluttertoast.showToast(
        msg: "Move the map to change the marker location and get location details of that point...",
        toastLength: Toast.LENGTH_LONG);

    if (_currentLocation != null && _pinnedLocation == null) {
      _setCameraPosition(_currentLocation!.latitude, _currentLocation!.longitude);
    }

    _mapController!.addListener(() {
      if (_mapController!.isCameraMoving && _mapController!.symbols.isNotEmpty) {
  ...
  Future<void> _fetchLocationDetails(LatLng latLng) async {
    try {
      final data = await ApiService.fetchLocationDetails(latLng.latitude, latLng.longitude);
      setState(() {
        _placeResponse = data;
      });
    } catch (e) {
  ...
  void _shareLocation() {
    if (_pinnedLocation != null) {
      ...
      final locationUrl = 'https://example.com/your_path?lat=$lat&lng=$lng'; // Adjust the URL as needed
        ...
        children: [
          MaplibreMap(
            ...
            initialCameraPosition: CameraPosition(
              target: _currentLocation ?? LatLng(widget.latitude ?? 27.7192873, widget.longitude ?? 85.3238007),
              zoom: 14.0,
            ),
            styleString: "https://api.baato.io/api/v1/styles/retro?key=bpk.FhqCNwsqS3vQz6KNopKlBOhudT4A_oPf5yOE4OMGW9Lr",
          ),
          Center(
            child: Container(
              child: _isCameraMoving ? Image.asset('assets/ic_marker.png') : null,
            ),
          ),
          _placeResponse != null
              ? Positioned(
